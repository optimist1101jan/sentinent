...
```mermaid
graph TD
    UserInput[User Input] --> Hold[HOLD: temp RAM, not logged]
    Hold --> BuildPacket[Build Packet: pipeline/packet_builder.py]
    
    subgraph Context Injection
        BuildPacket --> Proximity{Proximity changed?}
        Proximity -- Yes --> InjectProx[Inject from proximity_manager.py]
        BuildPacket --> Memory{Memory intent?}
        Memory -- Yes --> FetchMem[Fetch from memory_loader.py]
    end
    
    InjectProx --> GeminiAPI
    FetchMem --> GeminiAPI
    ...
    Validate -- Yes --> Commit[COMMIT: log both, increment counter]
    Validate -- No --> Discard[DISCARD: retry]
    
    Commit --> TurnCheck{Turn == 5?}
    TurnCheck -- Yes --> Summarize[Summarize -> Index to brain.db + FAISS -> Reset]
```
